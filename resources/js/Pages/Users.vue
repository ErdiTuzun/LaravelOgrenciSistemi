<template>
    <v-layout class="rounded rounded-md full-height">
        <v-navigation-drawer style="border:transparent">
            <Sidebar></Sidebar>
        </v-navigation-drawer>

        <v-app-bar class="full-width">
            <DashboardHeader></DashboardHeader>
        </v-app-bar>

        <div class="d-flex align-center justify-center elevation-5 mt-5 mx-auto py-6 px-4 sm:px-6 lg:px-8 relative overflow-x-auto shadow-md sm:rounded-lg">
            <a href="/admin/user/register">
                <v-btn prepend-icon="mdi-account-plus" class="mb-2 bg-primary" variant="outlined">Yeni Kullanıcı Ekle
                </v-btn>
            </a>
            ...
            <v-card flat>
                ...
                <v-data-table
                    ...
                    <template v-slot:item.photo="{ item }">
                        <img v-if="item.photo !== null" :src="item.photo" style="max-width: 80px; max-height: 80px;"
                             alt="Fotoğraf">
...
export default {
    components: {Sidebar, DashboardHeader, Dashboard, AuthenticatedLayout},
    mixins: [usersMixin],
    data() {
        return {
